...
sfo_trees_df<-read_csv(here("data","sfo_trees", "sfo_trees.csv")) |>
  janitor::clean_names()
```
Note: the double colons is a "lazy load," pulling just a single function rather than loading an entire package
...
Write them below:
- How many of each observation by legal status


-Command+Option_I inserts a code chunk
```{r}
unique(sfo_trees_df$legal_status)

legal_status_tree_count <- sfo_trees_df |>
...
```{r}
ggplot(data = legal_status_tree_count) +
  geom_col(aes(x=reorder(legal_status, -tree_count), y=tree_count)) +
  labs(x = "Legal Status",
       y = "Count of Trees") +
  theme_light() +
...
- %in% selects only the specified row values (given in an array) from a certain column
```{r}
sfo_trees_df |>
...
- filter for only oak and pine trees
- summarize counts
...
NOTES
- str_detect function searches through a longer string for a certain string/word within
- ifelse within the mutate function is creating a column named "type" with the value of either "Oak" or "Pine". if the str_detect function finds the phrase "Oak" in the species column, it fills in "Oak". Otherwise (else), it fills in "Pine".
...
```
NOTES:
- spearate function separates the species string into two columns, scientific and common names, using the " :: " to divide them
- mutate is used to create a column named ca_native with T/F values based on whether the spp_sci name value is found in the list of california native species dataframe
...
```
NOTES:
-t test finds a statistically significant diference in the mean year that the 2 species were planted
